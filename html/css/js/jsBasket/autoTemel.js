...
let search = document.querySelector('#search')
  ...
  const render = (data)=>{
   ...
   const searchFn = function(){
    ...
    let filtered = cars.filter(b=>b.brand.toUpperCase().includes(searchInp.toUpperCase()))
  ...
  const renderFilters = function(){
    let markaiNP = marka.value
    let modelInp = model.value
    let cityInp = city.value
    let yearInp = year.value
    let purifyMarka = [...new Set(cars.map(b=>b.brand))]
    marka.innerHTML = `<option value="">All Brands</option>` + purifyMarka.map(m=>`<option value="${m}">${m}</option>`)
    console.log(markaiNP);
    let purifyModel = [...new Set(cars.map(b=>b.model))]
    model.innerHTML = `<option value="">All Models</option>` + purifyModel.map(m=>`<option value="${m}">${m}</option>`)
    console.log(modelInp);
    let purifyCity = [...new Set(cars.map(b=>b.city))]
    city.innerHTML = `<option value="">All Cities</option>` + purifyCity.map(m=>`<option value="${m}">${m}</option>`)
    console.log(cityInp);
    let purifyYear = [...new Set(cars.map(b=>b.year))].sort((a,b)=>b-a)
    year.innerHTML = `<option value="">All Years</option>` + purifyYear.map(m=>`<option value="${m}">${m}</option>`)
    console.log(yearInp);
