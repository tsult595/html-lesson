...
let odomSearch = document.querySelector('#odomSearch')

    const render = (data)=>{
         cardWrapper.innerHTML = ""
        data.map(car=>{
            cardWrapper.innerHTML += `
              <div class="max-w-xs overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-md">
            <div class="relative">
                <img src="${car.images}"
                    alt="${car.brand} ${car.model}" class="h-52 w-full object-cover" />
                ...
                <div class="absolute right-3 top-3 flex flex-col gap-2">
                    <!-- Credit -->
                    <div class="flex items-center justify-center rounded-full bg-white/90 p-2 shadow">
                        <!-- credit card icon -->
                    ...
                    <div class="flex items-center justify-center rounded-full bg-white/90 p-2 shadow">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-700" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path d="M4 7h11l-3-3m3 3l-3 3M20 17H9l3 3m-3-3l3-3" stroke-width="2" stroke-linecap="round"
             ...
             <div class="p-4">
                    <div class="mb-2 flex items-start justify-between gap-2">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">${car.banType} • ${car.city}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs text-gray-700">
                        <div class="rounded-lg bg-gray-50 p-2">
                            <span class="block text-gray-400">Odometer</span>
                            <span class="font-medium">${car.odometer} ${car.odometerUnit}</span>
                        </div>
        
                        <div class="rounded-lg bg-gray-50 p-2">
                            <span class="block text-gray-400">Engine</span>
                            <span class="font-medium">${car.engine} L</span>
                        </div>
                    </div>
        
                    <button class="mt-3 w-full rounded-lg bg-black py-2 text-xs text-white hover:bg-gray-800">
                        View Details
                    </button>
             </div>
...
const searcher = function(){
 let searchVal = priceSearch.value.trim()
 let odomVal = odomSearch.value.trim()
 
 if(!searchVal && !odomVal){ 
     render(cars)
     return
 }
 
 // Filter by price and odometer independently
 let finden = cars.filter(car => {
     const matchPrice = !searchVal || String(car.price).includes(searchVal)
     const matchOdom = !odomVal || String(car.odometer).includes(odomVal)
     return matchPrice && matchOdom
 })
 render(finden)
}

const updateDependentFilters = () => {
    const brandVal = marka.value;

    // Фильтруем машины по выбранной марке (или все, если марка не выбрана)
    const filtered = brandVal ? cars.filter(car => car.brand === brandVal) : cars;
    console.log("Filtered cars for dependent filters:", filtered);

    const models = [...new Set(filtered.map(car => car.model))];
    const cityes = [...new Set(filtered.map(car => car.city))];
    const years  = [...new Set(filtered.map(car => car.year))].sort((a, b) => b - a);
    const currencyes = [...new Set(filtered.map(car=>car.currency))] 
    ...
    city.innerHTML =
        ...
        cityes.map(c => `<option value="${c}">${c}</option>`).join("");
        console.log(cityes);
    ...
    currencyes.map(c=> `<option value="${c}">${c}</option>`) 
};
...
const applyFilters = () => {
    ...
    const filtered = cars.filter(car =>
        ...
        (!cityVal  || car.city  === cityVal)  &&
        (!yearVal  || String(car.year) === String(yearVal)) &&
        (!currencyVal || car.currency === currencyVal)
    );
